 ...
 * Node.js + Express
 * Features: Booking API, Service Request API, Contact API,
 *           Email notifications via Nodemailer, SQLite DB, Rate Limiting
 */
...
// ─── DATABASE SETUP ──────────────────────────────────────────────────────────
const db = new Database(path.join(__dirname, 'grakhotel.db'));
...
// ─── MIDDLEWARE ───────────────────────────────────────────────────────────────
app.use(helmet({
  contentSecurityPolicy: false // allow inline scripts in the HTML frontend
}));
app.use(cors({
...
async function sendEmail(options) {
  ...
  try {
    await transporter.sendMail({
      from: `"GRAK Hotel Etah" <${process.env.SMTP_USER}>`,
      ...options
    });
  } catch (err) {
...
app.post('/api/booking', async (req, res) => {
  ...
  const price_night = roomPrice(room_type);
  const total_price = nights * price_night;
  ...
  try {
    const stmt = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    stmt.run(booking_ref, guest_name, email, phone, room_type, check_in, check_out, guests || 1, special_req || '');

    // Email to admin
    await sendEmail({
      to: process.env.ADMIN_EMAIL || 'admin@grakhotel.com',
      subject: `New Booking [${booking_ref}] — ${room_type} — ${guest_name}`,
      html: bookingEmailHtml({ booking_ref, guest_name, email, phone, room_type,
                               check_in, check_out, guests, special_req, nights, total_price }),
    });
...
app.post('/api/service-request', async (req, res) => {
  ...
  try {
    const stmt = db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    stmt.run(request_ref, guest_name, email, phone || '', room_no || '',
             service_cat, priority || 'Normal', preferred_dt || '', details);
    ...
    return res.status(201).json({
      success: true,
      message: 'Your service request has been submitted. Our team will respond within 2 hours.',
      request_ref,
    });
  } catch (err) {
...
app.post('/api/contact', async (req, res) => {
  ...
  try {
    ...
    await sendEmail({
      ...
      html: `<p><b>From:</b> ${name} (${email})<br><b>Phone:</b> ${phone || 'N/A'}<br><b>Subject:</b> ${subject || 'General Enquiry'}</p><p>${message.replace(/\n/g,'<br>')}</p>`,
    });
...
app.get('/api/availability', (req, res) => {
  ...
  const conflict = db.prepare(`
    ...
    WHERE room_type = ? AND status != 'cancelled'
      AND check_in  < ?
      AND check_out > ?
  `).get(room_type, check_out, check_in);
  ...
  return res.json({
    ...
    check_out,
    available,
    message: available ? 'Room is available for your dates.' : 'Room is not available for selected dates. Please choose different dates.',
  });
});
...
<style>
  ...
  .logo em{font-style:italic;color:#E8D08A}
  .tagline{font-size:0.7rem;letter-spacing:0.2em;color:#9a9080;margin-top:0.3rem}
  .body{padding:2rem}
  ...
  .ref{background:#2c2c2c;border-left:3px solid #C9A84C;padding:1rem;margin:1.5rem 0;font-family:monospace;font-size:1rem;color:#C9A84C;letter-spacing:0.1em}
  ...
  .btn{display:inline-block;padding:0.8rem 2rem;background:#C9A84C;color:#0d0d0d;text-decoration:none;font-size:0.75rem;letter-spacing:0.15em;text-transform:uppercase;margin-top:1.5rem}
</style></head><body>
<div class="wrap">
  <div class="header">
    <div class="logo">GR<em>AK</em></div>
    <div class="tagline">ETAH'S PREMIER LUXURY HOTEL</div>
  </div>
  <div class="body">${content}</div>
  <div class="footer">
    GRAK Hotel • Civil Lines, Etah, Uttar Pradesh 207001<br>
    +91 98765 43210 • reservations@grakhotel.com • grakhotel.com
  </div>
</div></body></html>`;
}
...
function bookingEmailHtml(d) {
  return emailWrapper(`
    ...
    <div class="row"><span class="label">Guests</span><span class="val">${d.guests}</span></div>
...
function bookingConfirmHtml(d) {
  return emailWrapper(`
    ...
    <p style="color:#9a9080;margin-bottom:1.5rem">Dear ${d.guest_name}, thank you for choosing GRAK Hotel. Your reservation has been received and is pending confirmation from our team.</p>
    <div class="ref">Booking Ref: ${d.booking_ref}</div>
    ...
    <div class="row"><span class="label">Nights</span><span class="val">${d.nights}</span></div>
    <div class="row"><span class="label">Estimated Total</span><span class="val">₹${d.total_price.toLocaleString('en-IN')}</span></div>
    <p style="color:#9a9080;font-size:0.8rem;margin-top:1.5rem">Our reservations team will contact you within 2 hours to confirm your stay. For any queries, email <a href="mailto:reservations@grakhotel.com" style="color:#C9A84C">reservations@grakhotel.com</a> or call +91 98765 43210.</p>
  `);
}

function serviceEmailHtml(d) {
  return emailWrapper(`
    <h2>Guest Service Request — ${d.priority || 'Normal'} Priority</h2>
    ...
    <div class="row"><span class="label">Guest Name</span><span class="val">${d.guest_name}</span></div>
    <div class="row"><span class="label">Room/Suite</span><span class="val">${d.room_no || 'Not specified'}</span></div>
    <div class="row"><span class="label">Email</span><span class="val">${d.email}</span></div>
    <div class="row"><span class="label">Phone</span><span class="val">${d.phone || 'Not provided'}</span></div>
    <div class="row"><span class="label">Preferred Date</span><span class="val">${d.preferred_dt || 'Flexible'}</span></div>
...
function serviceAckHtml(d) {
  return emailWrapper(`
    ...
    <p style="color:#9a9080;margin-bottom:1.5rem">Dear ${d.guest_name}, your request has been received by our concierge team and will be responded to within 2 hours.</p>
    <div class="ref">Request Ref: ${d.request_ref}</div>
    <div class="row"><span class="label">Service Category</span><span class="val">${d.service_cat}</span></div>
    <div class="row"><span class="label">Priority</span><span class="val">${d.priority || 'Normal'}</span></div>
    <p style="color:#9a9080;font-size:0.8rem;margin-top:1.5rem">For urgent matters, please call us directly at <span style="color:#C9A84C">+91 98765 43210</span>.</p>
  `);
}
